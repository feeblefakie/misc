...
ib_trx_t ib_trx;
//ib_crsr_t crsr_l1_s, crsr_l1_e, crsr_l3_o, crsr_l3_c, crsr_l3_ok, crsr_l5_c, crsr_l5_n, crsr_l6_o, crsr_l6_s, crsr_l6_e, crsr_ji;
//ib_tpl_t tpl_l1_s, tpl_l1_e, tpl_l3_o, tpl_l3_c, tpl_l3_ok, tpl_l5_c, tpl_l5_n, tpl_l6_o, tpl_l6_s, tpl_l6_e, tpl_ji;
bool use_ji = false;
int num_threads;
pthread_t *tid;
thread_arg_t *arg;
...
{
  if (argc != 4) {
    std::cerr << argv[0] << " orderdate use_ji(1/0) num_threads" << std::endl;
    exit(1);
  }
  ...
  arg = new thread_arg_t[num_threads];
...
void q3(void)
{
  ...
  while (err == DB_SUCCESS) {
    ...
    if (orderdate >= pred_o_orderdate) {
      break;
    }
    ib_tuple_read_u32(tpl_l3_o, 1, &pos);
    ib_tuple_read_u32(tpl_l3_o, 2, &freq);

    uint32_t *range = new uint32_t[num_threads];
    div_t q = div(freq, num_threads);
    ...
    for (int i = 0; i < num_threads; ++i) {
      ...
      arg[i].pos = cur_pos;
      arg[i].freq = range[i];
      if (pthread_create(&tid[i], NULL, sliced_q3, (void *) &arg[i]) != 0) {
...
sliced_q3(void *p)
{
  ...
  std::vector<im_val_t> pos_custkey_array = get_l3_custkeys_from_sequences(arg->pos, arg->freq);
...
{
  ib_err_t err;
  ib_crsr_t crsr_l6_o, crsr_l6_e, crsr_l6_s, index_crsr;
  ib_tpl_t tpl_l6_o, tpl_l6_e, tpl_l6_s;
...
{
  ib_err_t err;
  ib_crsr_t crsr_l1_s, crsr_l1_e, crsr_ji;
  ib_tpl_t tpl_l1_s, tpl_l1_e, tpl_ji;
  std::vector<im_agg_t> extendedprice_array;
...
{
  ib_err_t err;
  ib_crsr_t crsr_l5_c, crsr_l5_n, index_crsr;
  ib_tpl_t tpl_l5_c, tpl_l5_n;
