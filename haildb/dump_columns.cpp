...
void dump_columns(ib_tpl_t tpl);
...
bool init_db(void)
{
  ...
  err = ib_cfg_set_text("data_home_dir", "/data/local/hiroyuki/db/");
  err = ib_cfg_set_text("log_group_home_dir", "/data/local/hiroyuki/db/");
...
dump_columns(ib_tpl_t tpl)
{
  ...
  for (i = 0; i < n_cols; ++i) {
    ...
    if (ib_col_meta.type == IB_SYS) {
      std::cout << "SYS(" << ib_col_meta.type_len << ")";
      continue;
