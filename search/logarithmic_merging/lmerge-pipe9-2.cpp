...
int main(int argc, char *argv[])
{
  ...
  pthread_mutex_init(&flusher_mutex, NULL);
  pthread_mutex_init(&fin_mutex, NULL);
  pthread_cond_init(&flush_cond, NULL);
  pthread_cond_init(&flusher_cond, NULL);
  ...
  pthread_t ftid;
  if (pthread_create(&ftid, NULL, flusher, NULL) != 0) {
      perror("pthread_create");
      exit(1);
  }
...
void *flusher(void *p)
{
  //pthread_mutex_lock(&flusher_mutex);
  pthread_mutex_lock(&flush_buffer_queue_mutex);
  while (1) {
    pthread_cond_wait(&flusher_cond, &flush_buffer_queue_mutex);
    //pthread_cond_wait(&flusher_cond, &flusher_mutex);
    //pthread_mutex_lock(&flush_buffer_queue_mutex);
    Z0_flush(indexes);
    //pthread_mutex_unlock(&flush_buffer_queue_mutex);
    if (is_last_flush) { break; } // this is set by the starting thread only.
  }
  //pthread_mutex_unlock(&flusher_mutex);
  pthread_mutex_unlock(&flush_buffer_queue_mutex);
