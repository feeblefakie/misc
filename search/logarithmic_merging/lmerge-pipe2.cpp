...
#define FLUSH_THRESHOLD 64*1024*1024
...
int main(int argc, char *argv[])
{
  ...
  if (pthread_create(&tid, NULL, index_memory, NULL) != 0) {
    perror("pthread_create");
    exit(1);
  }

  indexes.push_back(false);
  bool Z0_exists = false;
  ...
  while (1) {
    ...
    // memory inversion done here
    
    // flush Z0 first (no pipelined with in-memory inversion)
    if (!Z0_exists) {
      dump(Z0, 0);
      std::cout << "dump Z0" << std::endl;
      Z0_exists = true;
    } else {
      std::cout << "rename Z0 to I0. and dump Z0" << std::endl;
      // rename Z0 with I0
      if (rename("Z0.bidx", "I0.bidx") < 0) {
        std::cerr << "rename failed" << std::endl;
      }
      if (rename("Z0.data", "I0.data") < 0) {
        std::cerr << "rename failed" << std::endl;
      }
      // dump Z0 with name "Z0"
      dump(Z0, 0);
      indexes[0] = true;
      is_multiway = true;
    }

    // another memory inversion starts
    if (!is_all_process_done) {
      if (pthread_create(&tid, NULL, index_memory, NULL) != 0) {
        perror("pthread_create");
        exit(1);
      }
...
void *index_memory(void *p)
{
  ...
  std::cout << "index_memory starts for Z0" << std::endl;
  ...
  std::cout << "index_memory ends for Z0 - time: " << t2 -t1 << std::endl;
...
// TODO: modify this with merging disi-resident Z0 with I*
// currently treating in memory Z0. 
void pipeline_merge_linear(pos_lists_t &Z0, uint32_t level)
{
  ...
  if (!bt->open("Z0", Lux::IO::DB_RDONLY)) {
    std::cerr << "open error for Z0" << std::endl;
    perror("Z0");
    exit(1);
  }
  ...
  if (!bt->first(c)) {
    ...
    if (!bt->cursor_get(c, &key, &val, Lux::IO::SYSTEM)) {
      ...
      current_key[0] = std::string((char *) key->data, key->size);
      current_data[0] = val;
      bt->clean_data(key);
    }
  }
