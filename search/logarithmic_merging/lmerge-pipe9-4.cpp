...
void pipeline_merge_linear(uint32_t level, bool create_tmp_index);
...
void *gradual_merger(void *p);
void pipeline_merge_linear_gradual(uint32_t target_level);
...
pthread_mutex_t gm_mutex;
pthread_mutex_t gmint_mutex;
pthread_cond_t flush_cond;
pthread_cond_t gm_cond;
pthread_cond_t gmint_cond;
...
bool flush_done[NUM_FLUSH_BUFFER];
bool is_memory_process_done = false;
...
int main(int argc, char *argv[])
{
  ...
  pthread_mutex_init(&gmint_mutex, NULL);
  pthread_cond_init(&flush_cond, NULL);
  pthread_cond_init(&gm_cond, NULL);
  pthread_cond_init(&gmint_cond, NULL);
  ...
  pthread_t gm_tid;
  if (pthread_create(&gm_tid, NULL, gradual_merger, NULL) != 0) {
      perror("pthread_create");
      exit(1);
  }
  ...
  is_memory_process_done = true;
  pthread_mutex_lock(&gm_mutex);
  pthread_mutex_unlock(&gm_mutex);
  pthread_cond_signal(&gm_cond);
  if (pthread_join(gm_tid, &ret)) {
    perror("pthread_join");
  }
...
void *index_memory(void *p)
{
  ...
  std::vector<pos_lists_t *> queued_list;
  ...
  while (1) {
    ...
    */
    ...
    if (used_buffer_size > FLUSH_THRESHOLD) {
      ...
      std::cout << "in-memory process [" << id << "] : " << t3 - t5 << " (s)" << std::endl;
      pthread_mutex_lock(&flush_buffer_queue_mutex);
      double t4 = gettimeofday_sec();
      std::cout << "in-memory process [" << id << "] is waited : " << t4 - t3 << " (s)" << std::endl;
      /*
      if (!queued_list.empty() && flush_done[id]) {
        for (std::vector<pos_lists_t *>::iterator itr = queued_list.begin();
             itr != queued_list.end(); ++itr) {
          delete *itr;
        }
        queued_list.clear();
        flush_done[id] = false;
      }
      queued_list.push_back(Z0);
      */
      flush_buffer_queue.push_back(Z0);
      if (flush_buffer_queue.size() == NUM_FLUSH_BUFFER) {
...
void *gradual_merger(void *p)
{
  pthread_mutex_lock(&gm_mutex);
  while (1) {
    pthread_cond_wait(&gm_cond, &gm_mutex);
    if (is_memory_process_done) {
      break;
    }
    pipeline_merge_linear_gradual(target_level);
    if (unlink("TMP.bidx") < 0) { perror("unlink"); }
    if (unlink("TMP.data") < 0) { perror("unlink"); }
    for (int i = GRADUAL_LEVEL; i < target_level; ++i) {
      indexes[i] = false;
      char fname1[256];
      char fname2[256];
    ...
    if (indexes.size() != target_level + 1) {
      indexes.push_back(false);
    }
    indexes[target_level] = true;
  }
  pthread_mutex_unlock(&gm_mutex);
  return NULL;
}
...
void Z0_flush(std::vector<bool> &indexes)
{
  ...
  //pthread_mutex_lock(&gmint_mutex);
  ...
  while (1) {
    ...
    int first_merge_level = pipeline_level;
    bool temporary_index = false;
    if (pipeline_level > GRADUAL_LEVEL) {
      pthread_mutex_lock(&gm_mutex); // wait until the gradual merge finished
      pthread_mutex_unlock(&gm_mutex);
      first_merge_level = GRADUAL_LEVEL;
      temporary_index = true;
    }

    pipeline_merge_linear(first_merge_level, temporary_index);
    for (int k = 0; k < first_merge_level; ++k) {
      indexes[k] = false;
      char fname1[256];
      char fname2[256];
    ...
    if (pipeline_level > GRADUAL_LEVEL) {
      // second merge is gradual
      target_level = pipeline_level;
      pthread_cond_signal(&gm_cond);
    }
  ...
  // time consuming, so should be cleard by each process
  ///*
  double t3 = gettimeofday_sec();
  ...
  double t4 = gettimeofday_sec();
  std::cout << "cleaning: " << t4 - t3 << std::endl;
  ...
  */
  ...
  double t2 = gettimeofday_sec();
  //pthread_mutex_unlock(&gmint_mutex);
...
#define IiNEXT(idx) \
  ...
  double t2 = gettimeofday_sec();
  std::cout << level+1 << "-way merge: " << t2 - t1 << " (s)" << std::endl;
}

void pipeline_merge_linear_gradual(uint32_t target_level)
{
  std::cout << "pipelining merge(gradual) - level: " << target_level << std::endl;
  double t1 = gettimeofday_sec();

  int num_indexes = target_level - GRADUAL_LEVEL + 1; // 1 for TMP

  // couter[0] and cursor_finished[0] for Z0
  int counter[num_indexes];
  bool cursor_finished[num_indexes];
  ...
  std::vector<Lux::IO::cursor_t *> btc;
  //str_list current_key;
  std::string current_key[num_indexes];
  //data_list current_data;
  Lux::IO::data_t *current_data[num_indexes];
  //current_key.reserve(level+1);
  //current_data.reserve(level+1);
  ...
  for (int i = 0; i < num_indexes; ++i) {
    ...
    if (i == num_indexes-1) {
      ...
      sprintf(fname, "I%d", i+GRADUAL_LEVEL);
    }
    if (!bt->open(fname, Lux::IO::DB_RDONLY)) {
      std::cerr << "open error for " << fname << std::endl;
      perror(fname);
  ...
  char fname[256];
  memset(fname, 0, 256);
  sprintf(fname, "I%d", target_level);
  if (!btI->open(fname, Lux::IO::DB_CREAT)) {
    std::cerr << "open error for " << fname << std::endl;
    perror(fname);
  }
...
#define IiNEXTG(idx) \
  if (!bts[idx]->next(btc[idx])) { \
    cursor_finished[idx] = true; \
  } else { \
    Lux::IO::data_t *key; \
    Lux::IO::data_t *val; \
    if (!bts[idx]->cursor_get(btc[idx], &key, &val, Lux::IO::SYSTEM)) { \
      std::cerr << "cursor_get failed" << std::endl; \
    } \
    current_key[idx] = std::string((char *) key->data, key->size); \
    current_data[idx] = val; \
    bts[idx]->clean_data(key); \
  }
  ...
  */
  ...
  double t2 = gettimeofday_sec();
  std::cout << num_indexes << "-way merge: " << t2 - t1 << " (s)" << std::endl;
}
