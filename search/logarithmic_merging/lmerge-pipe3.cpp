...
std::ifstream fin[NUM_MEM_THREADS];
...
int main(int argc, char *argv[])
{
  ...
  for (int i = 0; i < NUM_MEM_THREADS; ++i) {
    ...
    fin[i].open(argv[i+1], std::ios::in);
    arg[i].id = i;
...
void Z0_flush(std::vector<bool> &indexes, pos_lists_t &Z0, int gsn)
{
  ...
  while (1) {
    ...
    if (is_pipelining) {
      ...
      if (pipeline_level >= 1) {
        ...
        if (pipeline_level + 1 != indexes.size()) {
          indexes.push_back(false);
        }
  ...
  std::cout << "flushing end ... " << gsn << std::endl;
