...
void Z0_flush(std::vector<bool> &indexes, pos_lists_t &Z0, int gsn)
{
  ...
  if (flush_wait != 1) { // when only one waiting, do flushing without order
    while (gsn != flush_queue[0]) {
      pthread_cond_wait(&flush_cond, &flush_mutex);
    }
  }
  if (gsn != flush_queue[0]) {
    std::cout << "OUT OF ORDER FLUSHING OCCURS!!!" << std::endl;
  }
