...
#define BUFFER_LIMIT 64*1024*1024
...
pthread_cond_t cond; // condition variable for waiting for memory inversion
...
int main(int argc, char *argv[])
{
  ...
  arg->Z0_offset = Z0_offset;
  arg->counter = 1;
  if (pthread_create(&tid, NULL, index_memory, (void *) arg) != 0) {
  ...
  while (1) {
    ...
    // flush for the inversed buffer is not done yet
    pthread_mutex_lock(&mutex);
    is_flush_done = false;
    pthread_mutex_unlock(&mutex);
    
    // another memory inversion starts
    arg->Z0_offset = Z0_offset + 1;
    arg->counter++;
    if (pthread_create(&tid, NULL, index_memory, (void *) arg) != 0) {
      perror("pthread_create");
      exit(1);
    ...
    if (is_all_process_done) {
      // last merge
      Z0_flush(indexes, Z0[Z0_offset % 2]);
      break;
    } else {
      Z0_flush(indexes, Z0[Z0_offset % 2]);
    }
    ...
    // flushing done, so sending a signal in-memory indexing thread 
    std::cout << "seding signal ..." << std::endl;
...
void *index_memory(void *p)
{
  ...
  int off = arg->Z0_offset % 2;
  double t1 = gettimeofday_sec();
  std::cout << "index_memory starts for Z0-" << off << std::endl;
  ...
  while (getline(fin, line)) {
    ...
    */
    ...
    for (pos_lists_t::iterator itr = Ztmp.begin();
      ...
      LMergeAddToken(indexes, Z0[off], pos, used_buffer_size);
    }
    //std::cout << "doc: " << doc_id << std::endl;
    ++doc_id;
    pos_lists_t().swap(Ztmp);

    // first buffer is just flushed without wating
    if (arg->counter != 1 && used_buffer_size > (BUFFER_LIMIT - FLUSH_THRESHOLD)) {
      // can't store any more in this moment for limited buffer size
      //pthread_cond_wait 
      pthread_mutex_lock(&mutex);
      if (!is_flush_done) {
  ...
  std::cout << "index_memory ends for Z0-" << off << " - time: " << t2 -t1 << std::endl;
