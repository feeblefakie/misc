...
#define NUM_MEM_THREADS 7
#define NUM_EXTRA_MEMORY_INDEX 20
...
void *flush_memory_index(void *p);
int get_global_seq_no(void);

bool is_pipelining = false;
bool is_process_done[NUM_MEM_THREADS];
...
int main(int argc, char *argv[])
{
  ...
  pthread_cond_init(&flush_queue_cond, NULL);
  ...
  if (pthread_create(&flush_tid, NULL, flush_memory_index, NULL) != 0) {
  ...
  while (1) {
    ...
    */
    ...
    if (used_buffer_size > FLUSH_THRESHOLD) {
      ...
      while (1) {
        if (flush_queue.size() < NUM_EXTRA_MEMORY_INDEX) {
          flush_queue.push_back(Z0);
          break;
        }
        pthread_cond_wait(&flush_queue_cond, &flush_queue_mutex);
      }
...
void *flush_memory_index(void *p)
{
  ...
  while (1) {
    // break if all done and queue is empty
    pthread_mutex_lock(&flush_queue_mutex);
    if (flush_queue.empty()) {
      bool is_all_done = true;
      for (int i = 0; i < NUM_MEM_THREADS; ++i) {
        if (!is_process_done[i]) {
          is_all_done = false;
          break;
        }
      }
      ...
      if (is_all_done) {
        ...
        timeout.tv_nsec = 0;
        pthread_cond_timedwait(&flush_cond, &flush_mutex, (const timespec *) &timeout);
      }
    } else {
      pos_lists_t *Z0 = flush_queue.at(0);
      pthread_mutex_unlock(&flush_queue_mutex);
      Z0_flush(indexes, *Z0);
      Z0->clear();
      delete Z0;
      ...
      pthread_mutex_unlock(&flush_queue_mutex);
      pthread_cond_broadcast(&flush_queue_cond);
    }
  }
