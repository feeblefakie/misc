...
#define NUM_FLUSH_BUFFER 1 // for buffers after in-memory inversion
...
void pipeline_merge_linear(uint32_t level)
{
  ...
  std::vector< std::vector<uint8_t> > gsn_markers;
  for (int i = 0; i < num_flush_buffer; ++i) {
    std::vector<uint8_t> gsn_marker;
    vb_encode_num(0, gsn_marker);
    vb_encode_num(gsn, gsn_marker);
    gsn_markers.push_back(gsn_marker);

    // assign gsn
    ++gsn;
  }
  reverse(gsn_markers.begin(), gsn_markers.end());
...
#define IiNEXT(idx) \
  ...
  */
  for (int i = num_flush_buffer-1; i >= 0 ; --i) {
    itrs.push_back(flush_buffer_queue[i]->begin());
    itr_ends.push_back(flush_buffer_queue[i]->end());
    int idx = num_flush_buffer-i-1;
    Z0NEXT(idx);
  }
  ...
  while (1) {
    ...
    if (is_set) {
      ...
      if (num_smallest > 1) {
        ...
        for (int i = size - 1; i >= (int ) (size - num_smallest); --i) {
          int idx = smallest_i[i];

          // for gsn marker from
          if (idx < num_flush_buffer) {
            char *m = (char *) &(gsn_markers[idx][0]);
            memcpy(p, m, gsn_markers[idx].size());
            p += gsn_markers[idx].size();
            /*
            std::vector<uint32_t> tmp = vb_decode_nodiff((uint8_t *) m, gsn_markers[idx].size());
            std::cout << "tmp[0]: " << tmp[0] << std::endl;
            std::cout << "tmp[1](GSN): " << tmp[1] << std::endl;
            */
          }
          // for gsn marker here
        ...
        if (idx < num_flush_buffer) {
          Lux::IO::data_t new_val;
          new_val.size = val->size + gsn_markers[idx].size();
          new_val.data = new char[new_val.size];
          char *p = (char *) new_val.data;
          char *m = (char *) &(gsn_markers[idx][0]);
          memcpy(p, m, gsn_markers[idx].size());
          p += gsn_markers[idx].size();
          memcpy(p, (char *) val->data, val->size);
          btI->put(&key, &new_val);
          delete [] (char *) new_val.data;
        } else {
          btI->put(&key, val);
        }
        if (idx < num_flush_buffer) {
