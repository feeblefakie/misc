...
pthread_cond_t flush_buffer_queue_cond;
...
std::deque<pos_lists_t *> flush_buffer_queue;
...
int main(int argc, char *argv[])
{
  ...
  pthread_cond_init(&flush_buffer_queue_cond, NULL);
  ...
  while (1) {
    ...
    */
    ...
    if (used_buffer_size > FLUSH_THRESHOLD) {
      ...
      while (1) {
        if (flush_buffer_queue.size() < NUM_FLUSH_BUFFER_MAX) {
          flush_buffer_queue.push_back(Z0);
          pthread_mutex_unlock(&flush_buffer_queue_mutex);
          break;
        }
        pthread_cond_wait(&flush_buffer_queue_cond, &flush_buffer_queue_mutex);
      }

      pthread_mutex_lock(&flush_buffer_queue_mutex);
      if (flush_buffer_queue.size() == NUM_FLUSH_BUFFER ||
          flush_buffer_queue.size() == NUM_FLUSH_BUFFER * 2) {
...
void Z0_flush(std::vector<bool> &indexes)
{
  ...
  pthread_mutex_lock(&flush_buffer_queue_mutex);
  double t3 = gettimeofday_sec();
  std::deque<pos_lists_t *>::iterator itr_end = flush_buffer_queue.end();
  int j = 0;
  for (std::deque<pos_lists_t *>::iterator itr = flush_buffer_queue.begin();
      itr != itr_end; ++itr, ++j) {
    if (j == NUM_FLUSH_BUFFER) {
      break;
    }
    //(*itr)->clear();
    delete *itr;
  ...
  for (int i = 0; i < NUM_FLUSH_BUFFER; ++i) {
    flush_buffer_queue.pop_front();
  }
  //flush_buffer_queue.clear();
  pthread_mutex_unlock(&flush_buffer_queue_mutex);
  pthread_cond_broadcast(&flush_buffer_queue_cond);
  double t2 = gettimeofday_sec();
